  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-fade-in">
      <div className="space-y-1 md:space-y-2">
        <h1 className="text-2xl md:text-3xl font-bold">Welcome back, {myProfile.name.split(' ')[0]}</h1>
        <p className="text-muted-foreground text-sm md:text-lg">Here's your schedule for the upcoming days</p>
      </div>
        ...
        {dates.map((date) => {
          ...
          return (
            <button 
              ...
              className={cn(
                'flex flex-col items-center min-w-[60px] md:min-w-[80px] p-2 md:p-3 rounded-xl border-2 transition-all shadow-card shrink-0', 
                dateStr === selectedDateStr ? 'border-primary bg-primary/5' : 'border-border/50 hover:border-primary/50 bg-card', 
                isToday(date) && dateStr !== selectedDateStr && 'ring-2 ring-primary/30'
              )}
            >
              <span className="text-[10px] md:text-xs font-medium text-muted-foreground uppercase">{format(date, 'EEE')}</span>
              <span className={cn('text-lg md:text-xl font-bold', isToday(date) && 'text-primary')}>{format(date, 'd')}</span>
              <div className="w-full h-1 md:h-1.5 rounded-full bg-muted mt-1.5 md:mt-2 overflow-hidden">
                <div 
                  className={cn(
                    'h-full rounded-full', 
        ...
        <Card className="lg:col-span-1 shadow-card rounded-2xl">
          ...
          <CardContent className="space-y-3 md:space-y-4 px-4 md:px-6">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-4 pt-2">
              <div className="text-center p-3 md:p-4 rounded-xl bg-secondary">
                <div className="text-xl md:text-2xl font-bold">{tasksForSelectedDay.length}</div>
                <div className="text-[10px] md:text-xs text-muted-foreground font-medium">Tasks</div>
              </div>
              <div className="text-center p-3 md:p-4 rounded-xl bg-secondary">
                <div className="text-xl md:text-2xl font-bold">{Math.max(0, (capacity?.totalCapacity || 0) - (capacity?.usedCapacity || 0))}h</div>
                <div className="text-[10px] md:text-xs text-muted-foreground font-medium">Available</div>
              </div>
            </div>
            <Button className="w-full rounded-xl text-sm" variant="outline" onClick={() => setShowAddTask(true)}>
        ...
        <Card className="lg:col-span-2 shadow-card rounded-2xl">
          <CardHeader className="pb-2 px-4 md:px-6">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm md:text-base">Tasks for this day</CardTitle>
              <span className="text-[10px] md:text-xs text-muted-foreground">Drag to reorder</span>
            </div>
          </CardHeader>
          <CardContent className="px-4 md:px-6">
            {tasksForSelectedDay.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 md:py-12 text-muted-foreground">
                <Clock className="h-10 w-10 md:h-12 md:w-12 mb-2 md:mb-3 opacity-30" />
                <p className="text-xs md:text-sm font-medium">No tasks scheduled</p>
              </div>
            ) : (
      ...
      <Dialog open={showAddTask} onOpenChange={setShowAddTask}>
        <DialogContent className="rounded-2xl mx-4 max-w-[calc(100%-2rem)] sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-lg md:text-xl font-bold">Add Your Own Task</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 md:space-y-4">
            <div className="space-y-2">
              <Label className="text-sm">Task Title</Label>
              <Input placeholder="What did you work on?" value={newTaskTitle} onChange={(e) => setNewTaskTitle(e.target.value)} className="h-10 md:h-11 rounded-xl" />
            </div>
            <div className="space-y-2">
              <Label className="text-sm">Description (optional)</Label>
              <Textarea placeholder="Brief description" value={newTaskDescription} onChange={(e) => setNewTaskDescription(e.target.value)} rows={2} className="rounded-xl" />
            </div>
            <div className="space-y-2">
              <Label className="text-sm">Hours Spent</Label>
              <Input type="number" min="0.5" max="24" step="0.5" placeholder="e.g., 2" value={newTaskHours} onChange={(e) => setNewTaskHours(e.target.value)} className="h-10 md:h-11 rounded-xl" />
            </div>
          </div>
          <DialogFooter className="gap-2 flex-col sm:flex-row">
            <Button variant="outline" onClick={() => setShowAddTask(false)} className="rounded-xl w-full sm:w-auto">Cancel</Button>
            <Button onClick={handleAddSelfTask} disabled={!newTaskTitle || !newTaskHours || createTask.isPending} className="rounded-xl shadow-lg shadow-primary/25 w-full sm:w-auto">
